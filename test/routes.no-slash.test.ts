...
// Disable leading slashes in generated paths
const routes = createRoutePaths(dummyRoutes, { includeLeadingSlash: false });

describe("Route Management (without leading slash)", () => {
  ...
  it("should generate the correct path for user url encoded", () => {
    const path = generatePath(routes.user, { userId: "id with spaces" });
  ...
  it("should generate the correct path for posts route without parameters", () => {
